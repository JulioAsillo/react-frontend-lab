...
// Hash determinista de fila para reconciliar con localStorage (mismo algoritmo
// que useValidaciones.rowId — duplicado a propósito porque excel.js no debe
// importar de hooks de React).
function rowIdForExport(row) {
  let h = 0x811c9dc5;
  const keys = Object.keys(row).sort();
  for (const k of keys) {
    const v = row[k];
    const piece = `${k}\u0001${v === null || v === undefined ? "" : String(v)}\u0002`;
...
export function exportSheetValidaciones(rows, cols, getVal, getComentario, scenario, persistKey) {
 ...
 * desde v10).
 */
export function exportSheetPlano(rows, cols, persistKey) {
