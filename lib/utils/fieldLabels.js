...
export function getLabel(field) {
  ...
  // Sin etiqueta explícita ni global: avisamos en dev para cazar fugas de
  // nombres crudos del backend antes de que lleguen a producción.
  if (process.env.NODE_ENV !== "production") {
    console.warn(`[labels] columna sin etiqueta: "${field}" — muéstrala con un label explícito.`);
  }
  // Autoformat: snake_case o camelCase → palabras (último recurso en prod)
  return field
...
/**
 * labelFor — etiqueta visible de una columna con prioridad de fuentes:
 *   1. src.colLabels[key]  → label explícito colocado en la fuente (recomendado)
 *   2. getLabel(key)       → mapa global fieldLabels
 *   3. (dev) aviso + autoformat como último recurso
 *
 * El nombre crudo del backend nunca se usa como texto salvo fallback en prod.
 * Se pasa el objeto `src` completo (fuente de BD_SOURCES / PRIV_BD_SOURCES / etc.).
 */
export function labelFor(src, key) {
  const explicit = src && src.colLabels && src.colLabels[key];
