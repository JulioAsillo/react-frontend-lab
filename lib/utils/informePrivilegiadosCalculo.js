...
/**
 * informePrivilegiadosCalculo — v25.3
 *
 * Calcula automáticamente todos los campos del Informe de Privilegiados.
 *
 * ── CORRECCIONES v25.3 (antes daba todo en 0) ──────────────────────────────
 ...
 *   3. PORCENTAJES (v25.4 — fórmula de CUMPLIMIENTO confirmada por Julio):
 ...
 *
 *   4. MFA (nuevo hallazgo v25.4 — un solo escenario "Validación", una hoja):
...
// Slug de sheetKey — idéntico a DataTablePrivilegiados (ScenarioSheet)
function toSheetSlug(s) {
...
function normalizeSheets(rawData) {
  ...
  const out = {};
  for (const [k, v] of Object.entries(rawData)) if (Array.isArray(v)) out[k] = v;
  return out;
}
