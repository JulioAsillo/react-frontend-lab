...
export async function fetchReporte(endpoint, needsDate, fecha) {
  ...
  // Si el reporte usa fecha y el backend NO devolvió una fecha de corte propia,
  // la fecha de corte ES la fecha seleccionada por el usuario. Sin esto, el
  // store recibe fechaCorte=undefined → reportesStore borra `${pk}-fechaCorte`
  // de IDB → al cambiar de vista la fecha no se cachea y vuelve a "hoy".
  if (needsDate && fecha && !fechaCorte) {
    fechaCorte = fecha;
  }
